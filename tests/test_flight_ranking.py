"""Unit tests for the flight ranking engine (pure functions, no I/O)."""
...
def test_normalize_basic_fields():
    o = flight_ranking.normalize_offer(_raw())
...
def test_normalize_counts_stops():
    ...
    assert flight_ranking.normalize_offer(raw)["stops"] == 1
...
def test_normalize_baggage_is_minimum_across_segments():
    ...
    assert flight_ranking.normalize_offer(raw)["checked_bags"] == 0


def test_normalize_refundable_penalty():
    o = flight_ranking.normalize_offer(_raw(refundable=True))
...
def _norm(**kw):
    return flight_ranking.normalize_offer(_raw(**kw))


def test_matches_max_stops():
    direct = _norm(segments=[_seg("TLV", "ROM")])
    one_stop = _norm(segments=[_seg("TLV", "IST"), _seg("IST", "ROM")])
    assert flight_ranking.matches_preferences(direct, {"max_stops": 0})
    assert not flight_ranking.matches_preferences(one_stop, {"max_stops": 0})


def test_matches_refundable_only():
    assert not flight_ranking.matches_preferences(_norm(refundable=False), {"refundable_only": True})
    assert flight_ranking.matches_preferences(_norm(refundable=True), {"refundable_only": True})


def test_matches_min_checked_bags():
    assert not flight_ranking.matches_preferences(
        _norm(segments=[_seg("TLV", "ROM", checked=0)]), {"min_checked_bags": 1})
    assert flight_ranking.matches_preferences(
        _norm(segments=[_seg("TLV", "ROM", checked=2)]), {"min_checked_bags": 1})


def test_matches_airline_include_exclude():
    ba = _norm(airline="BA")
    assert flight_ranking.matches_preferences(ba, {"airlines_include": ["BA", "LY"]})
    assert not flight_ranking.matches_preferences(ba, {"airlines_include": ["LY"]})
    assert not flight_ranking.matches_preferences(ba, {"airlines_exclude": ["BA"]})


def test_matches_max_price():
    o = _norm(price="500.00")
    assert flight_ranking.matches_preferences(o, {"max_price": 600})
    assert not flight_ranking.matches_preferences(o, {"max_price": 400})
...
def test_expert_notes_flag_nonrefundable_and_no_checked_bag():
    ...
    notes = " | ".join(flight_ranking.expert_notes(o))
...
def test_recommend_sorts_and_limits_and_annotates():
    ...
    result = flight_ranking.recommend(offers, {}, limit=2)
...
def test_recommend_note_when_nothing_matches():
    ...
    result = flight_ranking.recommend(offers, {"refundable_only": True})
